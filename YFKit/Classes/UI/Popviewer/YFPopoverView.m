...
#import "YFPopoverView.h"
#import "YFPopoverViewCell.h"
...
float YFPopoverViewDegreesToRadians(float angle)
{
    return angle*M_PI/180;
}

@interface YFPopoverView () <UITableViewDelegate, UITableViewDataSource>
...
#pragma mark - Data
@property (nonatomic, copy) NSArray<YFPopoverAction *> *actions;
...
- (void)setStyle:(YFPopoverViewStyle)style
{
    _style = style;
    
    _tableView.separatorColor = [YFPopoverViewCell bottomLineColorForStyle:_style];
    
    if (_style == YFPopoverViewStyleDefault) {
...
- (void)initialize
{
    ...
    _actions = @[];
    _isUpward = YES;
    _style = YFPopoverViewStyleDefault;
    _arrowStyle = YFPopoverViewArrowStyleRound;
    ...
    [_tableView registerClass:[YFPopoverViewCell class] forCellReuseIdentifier:kPopoverCellReuseId];
...
- (void)showToPoint:(CGPoint)toPoint
{
    ...
    // 如果是菱角箭头的话, 箭头宽度需要小点.
    if (_arrowStyle == YFPopoverViewArrowStyleTriangle) {
        arrowWidth = 22.0;
    }
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, cornerRadius + maskTop)
                        radius:cornerRadius
                    startAngle:YFPopoverViewDegreesToRadians(180)
                      endAngle:YFPopoverViewDegreesToRadians(270)
                     clockwise:YES];
    // 箭头向上时的箭头位置
    if (_isUpward) {
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskTop + cornerRadius)
                        radius:cornerRadius
                    startAngle:YFPopoverViewDegreesToRadians(270)
                      endAngle:YFPopoverViewDegreesToRadians(0)
                     clockwise:YES];
    // 右下圆角
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:YFPopoverViewDegreesToRadians(0)
                      endAngle:YFPopoverViewDegreesToRadians(90)
                     clockwise:YES];
    // 箭头向下时的箭头位置
    if (!_isUpward) {
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:YFPopoverViewDegreesToRadians(90)
                      endAngle:YFPopoverViewDegreesToRadians(180)
                     clockwise:YES];
    [maskPath closePath];
    // 截取圆角和箭头
...
- (CGFloat)calculateMaxWidth
{
    ...
    UIFont *titleFont = [YFPopoverViewCell titleFont];
    
    for (YFPopoverAction *action in _actions) {
...
- (void)showToView:(UIView *)pointView withActions:(NSArray<YFPopoverAction *> *)actions
{
...
- (void)showToPoint:(CGPoint)toPoint withActions:(NSArray<YFPopoverAction *> *)actions
{
    if (!actions) {
        _actions = @[];
    } else {
...
{
    YFPopoverViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kPopoverCellReuseId];
    cell.style = _style;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    [UIView animateWithDuration:0.25f animations:^{
        ...
        YFPopoverAction *action = _actions[indexPath.row];
