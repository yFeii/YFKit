...
@interface YFWebView () <WKNavigationDelegate, WKUIDelegate, IMY_NJKWebViewProgressDelegate>
...
{
    return [self initWithFrame:frame usingUIWebView:false];
}
...
- (void)setDelegate:(id<YFWebViewDelegate>)delegate
{
    _delegate = delegate;
...
- (void)observeValueForKeyPath:(NSString*)keyPath ofObject:(id)object change:(NSDictionary*)change context:(void*)context
{
    ...
    if ([self.delegate respondsToSelector:@selector(yf_observeValueForKeyPath:ofObject:change:context:)]) {
        
        [self.delegate yf_observeValueForKeyPath:keyPath ofObject:object change:change context:context];
    }
}
...
#pragma mark - IMY_NJKWebViewProgressDelegate
...
- (nullable WKNavigation *)loadHTMLString:(NSString*)string baseURL:(NSURL*)baseURL
{
...
- (UIProgressView *)progressView {
    if (!_progressView) {
        ...
        _progressView.trackTintColor = UIColorHexMake(@"#f5f6f8");
    }
